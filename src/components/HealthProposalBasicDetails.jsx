...
<div className='proposer-details-div'>
      {data.map((item, index) =>{
let healthIssueHTML = ''
if (document.getElementById(`${item.relation}-healthIssue`) == 'yes') {

  healthIssueHTML =`  <div>
  <input type="checkbox" id="${item.relation}-cancer" name="${item.relation}-cancer"} value="cancer" />
  <label htmlFor="${item.relation}-cancer">Cancer</label>
  <br />
  <input type="checkbox" id="${item.relation}-heartDisease" name="${item.relation}-heartDisease" value="heartDisease" />
  <label htmlFor="${item.relation}-heartDisease">Cardiovascular/Heart Disease</label>
  <br />
</div>`
}
      return(
