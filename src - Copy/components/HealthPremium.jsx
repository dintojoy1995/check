...
const HealthPremium = () => {
  ...
  const inputdata = localStorage.getItem("health_age_details");
    ...
    const data = {
     input_data:
     [
     {"relation": "Self", "Age": 35, Gender:"Male"},
     {"relation": "Spouse", "Age": 33, Gender:"Female"},
      {"relation": "Son", "Age": 13, Gender:"Male"},
      {"relation": "Daughter", "Age": 11, Gender:"Female"},
       {"relation": "Son", "Age": 10, Gender:"Male"},
    ],
    proposer_data:{fullName:"dinto joy",Age:27,pincode:683576,mobileNumber:9072072009,Gender:"Female"},
    policyType,
    room_limit:"all",
    sumAssured,
    tenure
  }
  const filteredRelations = data.input_data.filter(relation => relation.relation !== "Son" && relation.relation !== "Daughter");
  const relationItems = filteredRelations.map(relation => relation.relation).join(", ");

 // Count the number of Sons and Daughters
 let sonCount = 0;
 let daughterCount = 0;
 data.input_data.forEach(relation => {
    ...
    <div className="container">
      <div className="row">
        <div className="col-md-5 mt-5">
          <span><h6>{data.input_data.length} Members</h6></span>
